...
##一.3.1 类和对象
###1.类
- 成员方法重载：一个类中有同名的成员方法，参数列表不同。重载的多种方法为一种功能提供多种实现。重载方法之间必须以不同的参数列表（数据类型，参数个数，**参数次序**（相同的数据类型和参数个数次序不同也重载））来区别。只有返回值不同不构成方法重载。只有形参的名称不同，不构成方法的重载。构造方法与普通方法一样可以重载。
如：   
